...
function generateRoster() {
  ...
  // Store in global
  window.currentRoster = dummyRoster;

  // Show UI sections
  document.getElementById("flight-info").style.display = "block";
  document.getElementById("roster-views").style.display = "block";
  document.getElementById("export-btn").disabled = false;

  // Call view update
  updateRosterViews(dummyRoster);
}
function exportRoster() {
  if (!window.currentRoster) {
    showToast("No roster to export.", "error");
    return;
  }

  const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(window.currentRoster, null, 2));
  const dlAnchorElem = document.createElement("a");
  dlAnchorElem.setAttribute("href", dataStr);
  dlAnchorElem.setAttribute("download", "flight_roster.json");
  dlAnchorElem.click();
}
...
cockpitSeats.innerHTML = `
  <div class="seat crew" title="${crew.name} - ${crew.seniority} Pilot">
      <div class="initials">${crew.name.split(" ").map(n => n[0]).join("")}</div>
      <div class="position">${crew.seniority}</div>
  </div>
`;
