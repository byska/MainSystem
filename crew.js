...
function updateFlightCrewManagementTable() {
  ...
  extendedCrewData.flightCrew.forEach((crew) => {
    ...
    <button 
      onclick="viewCrewMember('${crew.id}')" 
      style="padding: 4px 10px; font-size: 0.75rem; border-radius: 4px; background-color: #007bff; color: white; border: none; cursor: pointer;">
      View
    </button>
    <button 
      onclick="editCrewMember('${crew.id}')" 
      style="padding: 4px 10px; font-size: 0.75rem; border-radius: 4px; background-color: #6c757d; color: white; border: none; cursor: pointer;">
      Edit
    </button>
  </div>
                </td>
